<template>
    <div class="container mx-auto px-4 py-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-800">{{ $t('exchange_providers.title') }}</h1>
            <Button :label="$t('exchange_providers.create')" icon="pi pi-plus" @click="openCreateModal" />
        </div>

        <div class="card">
            <DataTable :value="providers" :loading="loading" stripedRows responsiveLayout="scroll">
                ...
                <Column field="isActive" header="Activo">
                    <template #body="{ data }">
                        <span v-if="data.isActive" class="px-2 py-1 text-xs rounded-full bg-emerald-100 text-emerald-800">Sí</span>
                ...
                <Column :header="$t('common.actions')" style="width: 10rem">
                    <template #body="{ data }">
                        <div class="flex gap-2">
                            <Button icon="pi pi-pencil" class="p-button-rounded p-button-warning p-button-text" @click="editProvider(data)" />
                            <Button
            ...
            <div v-if="!providers.length && !loading" class="text-center py-8 text-gray-500">
                {{ $t('common.no_data') }}
            </div>
        </div>

        <Dialog v-model:visible="showModal" :header="editingId ? $t('common.edit') : $t('exchange_providers.create')" :modal="true" :style="{ width: '580px' }">
            <div class="flex flex-col gap-4">
                ...
                <div class="form-group">
                    ...
                    <Select v-model="form.authType" :options="authTypeOptions" optionLabel="label" optionValue="value" class="w-full" />
                </div>
                ...
                <div class="flex items-center gap-4">
                    <div class="flex items-center">
                        <input type="checkbox" v-model="form.isActive" id="isActive" class="h-4 w-4 text-blue-600 rounded border-gray-300" />
                        <label for="isActive" class="ml-2 text-sm text-gray-700">Proveedor activo (solo uno)</label>
                    </div>
                    <div class="flex items-center">
                        <Select v-model="form.status" :options="[{label: t('common.active'), value:'active'},{label: t('common.inactive'), value:'inactive'}]" optionLabel="label" optionValue="value" class="w-32" />
                    </div>
                </div>
            </div>
            <template #footer>
...
const saveProvider = async () => {
    saving.value = true;
    try {
        const url = editingId.value ? `/exchange-providers/${editingId.value}` : '/exchange-providers';
...
const toggleStatus = async (provider) => {
    ...
    try {
        const response = await fetch(`/exchange-providers/${provider.id}/status`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
