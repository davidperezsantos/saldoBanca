        ...
        <template #content>
            <div class="container mx-auto px-4 py-8">
                <div class="flex items-center justify-between mb-6">
                    <Button v-if="common.can('oauth_clients:create')" :label="$t('oauth_clients.create')" icon="pi pi-plus" @click="openCreateModal" />
                </div>
                <DataTable :value="clients" :loading="loading" size="small" responsiveLayout="scroll" scrollable
                    ...
                    <Column field="identifier" :header="$t('oauth_clients.client_id')" style="min-width: 140px">
                        <template #body="{ data }">
                            <span class="px-2 py-1 text-xs font-mono bg-gray-100 rounded block truncate max-w-32"
                                :title="data.identifier">{{ data.identifier }}</span>
                        </template>
                    </Column>
                    <Column field="secret" :header="$t('oauth_clients.secret')" style="min-width: 120px">
                        <template #body="{ data }">
                            <span v-if="data.secret"
                                class="px-2 py-1 text-xs font-mono bg-gray-100 rounded block truncate max-w-36 select-all"
                                :title="data.secret">{{ data.secret }}</span>
                    ...
                    <Column field="grants" :header="$t('oauth_clients.grants')">
                        <template #body="{ data }">
                            <div class="flex flex-wrap gap-1">
                                <span v-for="g in data.grants" :key="g"
                                    class="px-2 py-0.5 text-xs bg-blue-100 text-blue-800 rounded-full">{{ g }}</span>
                            </div>
                        </template>
                    </Column>
                    <Column field="active" :header="$t('common.status')">
                        <template #body="{ data }">
                            <span
                                :class="['px-2 py-1 text-xs rounded-full', data.active ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800']">
                    ...
                    <Column :header="$t('common.actions')" style="width: 12rem">
                        <template #body="{ data }">
                            <div class="flex gap-2">
                                ...
                                <Button v-if="common.can('oauth_clients:edit')" icon="pi pi-pencil" class="p-button-rounded p-button-warning p-button-text"
                                    @click="editClient(data)" />
                                <Button v-if="common.can('oauth_clients:delete')" icon="pi pi-trash" class="p-button-rounded p-button-danger p-button-text"
                                    @click="deleteClient(data)" />
                                <Button v-if="common.can('oauth_clients:status')" :icon="data.active ? 'pi pi-power-off' : 'pi pi-check-circle'"
                ...
                <Dialog v-model:visible="showModal" :header="editingId ? $t('common.edit') : $t('oauth_clients.create')"
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="form-group">
                                ...
                                <InputText :value="editingId ? form.identifier : $t('oauth_clients.auto_generated')"
                                    class="w-full" readonly />
                            </div>
                            <div class="form-group">
                                <label class="form-label">{{ $t('oauth_clients.secret') }}</label>
                                <InputText
                                    :value="editingId && form.secret ? '••••••••' : $t('oauth_clients.auto_generated')"
                                    class="w-full" readonly :type="editingId ? 'password' : 'text'" />
                            </div>
                        </div>
                        <div class="flex items-center gap-6">
                            <div class="flex items-center">
                                <input type="checkbox" v-model="form.public" :id="'publicClient'"
                                    class="h-4 w-4 text-blue-600 rounded border-gray-300" @change="onPublicChange" />
                                <label :for="'publicClient'" class="ml-2 text-sm text-gray-700">{{
                                    $t('oauth_clients.public') }}</label>
                            </div>
                            <div class="flex items-center">
                                <input type="checkbox" v-model="form.allowPlainTextPkce" :id="'allowPkce'"
                                    class="h-4 w-4 text-blue-600 rounded border-gray-300" />
                                <label :for="'allowPkce'" class="ml-2 text-sm text-gray-700">{{
                                    $t('oauth_clients.allow_plain_text_pkce') }}</label>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <span v-for="grant in availableGrants" :key="grant" class="cursor-pointer">
                                    <span
                                        ...
                                        @click="toggleArray(form.grants, grant)">{{ grant }}</span>
                                </span>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div v-for="(uri, i) in form.redirectUris" :key="i" class="flex gap-2 mb-2">
                                <InputText v-model="form.redirectUris[i]" class="w-full"
                                    placeholder="https://ejemplo.com/callback" />
                                <Button icon="pi pi-times" class="p-button-rounded p-button-text p-button-danger"
                                    @click="form.redirectUris.splice(i, 1)" />
                            </div>
                            <Button :label="$t('oauth_clients.add_uri')" icon="pi pi-plus"
                                class="p-button-text p-button-sm" @click="form.redirectUris.push('')" />
                        </div>
                        <div class="form-group">
                            <label class="form-label">{{ $t('oauth_clients.scopes') }}</label>
                            <div v-for="(scopes, resource) in scopeGroups" :key="resource" class="mb-2">
                                <div class="text-xs font-semibold text-gray-400 uppercase mb-1">{{
                                    resourceLabel(resource) }}</div>
                                <div class="flex flex-wrap gap-2">
                                    <span v-for="scope in scopes" :key="scope" class="cursor-pointer" :title="scope">
                                        <span
                                            ...
                                            @click="toggleArray(form.scopes, scope)">{{ scopeLabel(scope) }}</span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div class="form-group flex items-center">
                            <input type="checkbox" v-model="form.active" :id="'activeClient'"
                                class="h-4 w-4 text-blue-600 rounded border-gray-300" />
                            <label :for="'activeClient'" class="ml-2 text-sm text-gray-700">{{ $t('common.active')
                            }}</label>
                        </div>
                    </div>
                    <template #footer>
                ...
                <Dialog v-model:visible="showDetail" :header="$t('oauth_clients.client_details')" :modal="true"
                    :style="{ width: '520px' }" :dismissableMask="true">
                    <div v-if="selectedClient" class="flex flex-col gap-3">
                        <div class="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 items-start">
                            ...
                            <div class="flex items-center gap-1 min-w-0">
                                <span
                                    class="font-mono text-sm bg-gray-100 px-2 py-1 rounded select-all cursor-pointer truncate max-w-full block"
                                    @click="copyToClipboard(selectedClient.identifier)">{{ selectedClient.identifier
                                    }}</span>
                                <i class="pi pi-copy text-gray-400 cursor-pointer hover:text-gray-600 shrink-0"
                                    @click="copyToClipboard(selectedClient.identifier)"></i>
                            </div>
                            ...
                            <template v-if="selectedClient.secret">
                                ...
                                <div class="flex items-center gap-1 min-w-0">
                                    <span
                                        class="font-mono text-sm bg-gray-100 px-2 py-1 rounded select-all cursor-pointer break-all min-w-0"
                                        @click="copyToClipboard(selectedClient.secret)">{{ selectedClient.secret
                                        }}</span>
                                    <i class="pi pi-copy text-gray-400 cursor-pointer hover:text-gray-600 shrink-0"
                                        @click="copyToClipboard(selectedClient.secret)"></i>
                                </div>
                            </template>
                            <template v-else>
                            ...
                            <div class="flex flex-wrap gap-1">
                                <span v-for="g in selectedClient.grants" :key="g"
                                    class="px-2 py-0.5 text-xs bg-blue-100 text-blue-800 rounded-full">{{ g }}</span>
                            ...
                            <div v-if="selectedClient.redirectUris?.length" class="flex flex-col gap-1 min-w-0">
                                <span v-for="uri in selectedClient.redirectUris" :key="uri"
                                    class="text-sm text-gray-600 break-all bg-gray-50 px-2 py-1 rounded">{{ uri
                                    }}</span>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-1">
                                <span v-for="s in selectedClient.scopes" :key="s" :title="s"
                            ...
                            <span
                                :class="['px-2 py-0.5 text-xs rounded-full w-fit', selectedClient.active ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800']">
...
import availableScopes from '@/static/availableScopes.json';
...
const resourceLabels = {
    ...
    exchange_providers: 'Proveedores de tasas de cambio',
    payment_gateways: 'Pasarelas de pago',
};

const scopeLabel = (scope) => t(`oauth_clients.scopes_labels.${scope}`);
const resourceLabel = (resource) => resourceLabels[resource] || resource;

// Sin default: cada cliente nuevo debe recibir explícitamente los scopes que necesita (mínimo privilegio).
...
const viewClient = (client) => {
    ...
    // El listado ya no trae el secreto real (queda enmascarado) — se pide bajo demanda solo
    // cuando se abre el detalle, en vez de viajar en cada carga de la tabla.
    if (client.secret) {
        common.ajax(urls.secretUrl.replace('__ID__', client.identifier), 'GET', null, (data) => {
            if (data.success && selectedClient.value === client) {
                selectedClient.value = { ...client, secret: data.data.secret };
            }
        }, onAjaxError);
    }
};
